...
// 소스코드를 분석하여 문제점을 발견하고 수정하라는 업무 과제를 받았습니다.
// 소스를 컴파일하여 로그를 보니 대부분 소스 코드 내 작성된 괄호가 개수는 맞지만 짝이 맞지 않은 형태로 
// 작성되어 오류가 나는 것을 알게 되었습니다. 
// 수정해야 할 소스 파일이 너무 많아서 고민하던 "콘"은 소스 코드에 작성된 모든 괄호를 뽑아서 올바른 
// 순서대로 배치된 괄호 문자열을 알려주는 프로그램을 다음과 같이 개발하려고 합니다.

// 용어의 설명
// '(' 와 ')' 로만 이루어진 문자열이 있을 경우, '(' 의 개수와 ')' 의 개수가 같다면 이를 균형잡힌 괄호 문자열이라고 부릅니다.
// 여기에 '('와 ')'의 괄호의 짝도 모두 맞을 경우에는 이를 올바른 괄호 문자열이라고 부릅니다.
...
// "균형잡힌 괄호 문자열" p가 매개변수로 주어질 때, 주어진 알고리즘을 수행해 "올바른 괄호 문자열"로 변환한 결과를 
  ...
  function solve(p)
  {
    ...
    for (let i = 0; i < p.length; i++) //2. Logic 수행
    {
      ...
      else
      {
        ...
        //이전 문자와 현재 문자가 같은 경우 ["(" == "("] or [")" == ")"] stack에 push한다.
          stack.push(now_char)
        else //다른 경우는 ["(" != ")"] or [")" != "("]인 경우
          stack.pop();
        ...
        {
          u = p.slice(0, i + 1);
            // p에서 0과 i+1을 반환한 값을 u에 넣는다.
          v = p.slice(i + 1);
            // p에서 i+1을 반환한 값을 v에 넣는다.
          break;
        }
      }
    }
    ...
    {
      u = u.slice(1 , u.length - 1); //4-4. Logic에서 첫 번째, 마지막 문자 제거, 미리 문자열 자르기를하여 
    // 혹시 모를 예외방지
